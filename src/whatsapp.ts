...
import { ChatIncoming } from './../node_modules/.prisma/client/index.d';
...
import { initStore, Store, useSession, transformPrisma } from '@ookamiiixd/baileys-store';
...
export async function createSession(options: createSessionOptions) {
  ...
  socket.ev.on('messages.upsert', async (m) => {
    ...
    if(fromMe === false){
      await prisma.chatIncoming.upsert({
        select: {pkId: true},
        create: {
          id: message.key.id!,
          sessionId: sessionId,
